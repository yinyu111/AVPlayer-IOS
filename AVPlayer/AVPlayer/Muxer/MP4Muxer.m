...
- (instancetype)initWithConfig:(MuxerConfig *)config {
    ...
    if (self) {
        _config = config;
        _muxerQueue = dispatch_queue_create("com.kaku.muxerQueue", DISPATCH_QUEUE_SERIAL); // 封装任务队列。
...
- (void)cancelWriting:(CMSampleBufferRef)sampleBuffer {
    // 取消写入。
    __weak typeof(self) weakSelf = self;
    dispatch_async(self.muxerQueue, ^{
        dispatch_semaphore_wait(weakSelf.semaphore, DISPATCH_TIME_FOREVER);
...
- (void)appendSampleBuffer:(CMSampleBufferRef)sampleBuffer {
    ...
    dispatch_async(self.muxerQueue, ^{
        ...
        if (!weakSelf.muxWriter) {
            ...
            //创建 Muxer 实例
            NSError *error = nil;
            BOOL success = [weakSelf _setupMuxWrite:&error];
            if (!success) {
...
- (BOOL)_setupMuxWrite:(NSError **)error {
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:self.config.outputURL.path]) {
        [[NSFileManager defaultManager] removeItemAtPath:self.config.outputURL error:nil];
    }
...
- (void) _avInterLeavedSample {
    ...
    if ((self.config.muxerType & MediaAudio) && (self.config.muxerType & MediaVideo)) { // 同时封装音频和视频。
        ...
        [self _appendAudioSample];
    } else if (self.config.muxerType & MediaVideo) { // 只封装音频。
        [self _appendVideoSample];
    }
}
