...
//kaku:使用提供的音频缓冲列表、时间戳、帧数和音频流描述创建一个 CMSampleBuffer 对象，这个对象可以用于进一步的处理或编码。
+ (CMSampleBufferRef)sampleBufferFromAudioBufferList:(AudioBufferList)buffers inTimeStamp:(const AudioTimeStamp *)inTimeStamp inNumberFrames:(UInt32)inNumberFrames description:(AudioStreamBasicDescription)description {
    ...
    // 1、创建音频流的格式描述信息。
    CMFormatDescriptionRef format = NULL;//kaku：用于存储音频流的格式描述信息。
    ...
    mach_timebase_info(&info);//kaku：这行代码填充了info结构体。mach_timebase_info函数返回了系统时钟的频率信息，用于将mach_absolute_time返回的时间转换为秒。numer和denom用于将mach_absolute_time的计数转换为纳秒。
    uint64_t time = inTimeStamp->mHostTime;//kaku：这里获取了一个音频帧的时间戳，mHostTime通常是一个由mach_absolute_time返回的值。
    // 转换为纳秒。
    time *= info.numer;
    time /= info.denom;
    // PTS。
    CMTime presentationTime = CMTimeMake(time, 1000000000.0f);//使用转换后的纳秒时间创建一个CMTime结构体，用于表示音频帧的呈现时间戳（PTS）。CMTimeMake函数的第二个参数是时间尺度，这里设置为1000000000.0f，表示1秒内有10亿个时间单位，即纳秒。
    // 对于音频，PTS 和 DTS 是一样的。
...
static OSStatus audioBufferCallBack(void *inRefCon,
    ...
    //kaku:使用 @autoreleasepool 来管理自动释放池，确保在块结束时释放对象。
    @autoreleasepool {
